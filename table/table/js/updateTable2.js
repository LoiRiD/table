...
function sortTr( team, place ) {
	...
	var tr1 = document.getElementById( teams[team].id );
	var tr2 = document.getElementById( -teams[team].id );
	for( var i = tr1.rowIndex - 2, j = tr2.rowIndex - 2; i >= place; i -= 2, j -= 2 ) {
		table.rows[i].parentNode.insertBefore( tr1, table.rows[i] );
		table.rows[i].parentNode.insertBefore( tr2, table.rows[j] );
	}
}
